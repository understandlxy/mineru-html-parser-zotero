...
如果你使用 Add-on Market for Zotero，需要等待本插件被 `syt2/zotero-addons-scraper` 索引仓库合并并刷新后，才能直接在市场里搜索到。
...
GitHub Release 发布只是第一步。Add-on Market for Zotero 还需要索引仓库收录插件信息。目前已提交到 `syt2/zotero-addons-scraper`：
...
等维护者合并并刷新数据源后，插件才会出现在市场搜索结果里。

### 为什么只能选一个 PDF？

这是当前版本的安全边界。MinerU 解析可能耗时较长，也会消耗 API 额度；一次只处理一个文件可以让进度、错误提示和附件归属更清楚。
...
可以通过 Zotero 扩展代理文件加载源码。Zotero profile 的 `extensions` 目录中创建一个文件，文件名为插件 ID：
...
脚本会读取 `addon/manifest.json` 中的版本号，并生成：

```text
dist\mineru-html-parser-0.1.48.xpi
```

打包脚本使用 .NET zip API，并显式写入 `/` 分隔的 zip entry 名称，避免 Windows 上打包出的 XPI 路径不兼容。

## 发布新版本

1. 修改 `addon/manifest.json` 中的 `version`。
2. 修改 `update.json` 中的 `version` 和 `update_link`。
3. 提交代码。
4. 推送与版本号一致的 tag，例如：
...
GitHub Actions 会自动打包 XPI，并把 XPI 与 `update.json` 上传到对应 Release。

## Project Summary

MinerU HTML Parser for Zotero is a Zotero bootstrapped plugin that uploads one selected local PDF attachment to the MinerU precise parsing API, downloads the generated HTML result, performs lightweight reading-oriented cleanup, and attaches the HTML back to the Zotero item.
